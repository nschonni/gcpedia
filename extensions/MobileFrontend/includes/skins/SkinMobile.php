...
/** GCChange Legend for this file
 * GCChange[1] - commented out if statements for beta cookies
 * GCChange[2] - removed link to Special:MobileOptions and replaced with Special:Preferences... also added Special:Favoritelist
 * GCChange[3] - added history and edit links if user is logged in
 * GCChange[4] - organized and added links to the navigation menu... added css to extensions/MobileFrontend/stylesheets/common/mf-navigation.css... also added icons to extensions/MobileFrontend/stylesheets/common/images/menu
 * GCChange[5] - added favorite's heart to top bar if logged in
 * GCChange[6] - changed links in the footer
 */
...
class SkinMobile extends SkinMobileBase {
	...
	protected function prepareTemplate( OutputPage $out ) {
		...
		if ( $device['supports_jquery'] ) {
			...
			// if ( $inBeta && $wgMFEnableResourceLoader ) { // GCChage[1]
...
mediawiki.hidpi' ), 'scripts', true, true );
		...
		// $tpl->set( 'settingsUrl', // GCChange[2]
		...
		$tpl->set( 'favoritesUrl',
			SpecialPage::getTitleFor( 'Favoritelist' )->
				getLocalUrl( array( 'returnto' => $this->getTitle()->getPrefixedText() ) )
		);
...
class SkinMobileTemplate extends BaseTemplate {
	...
	<div class='show <?php $this->html( 'articleClass' ); ?>' id='content_wrapper'>
			<?php //if ( $this->data['isBetaGroupMember'] && !$this->data['isSpecialPage'] ) { // GCChange[1] ?>
			<?php // GCChange[3] ?>
			<?php global $wgUser, $wgRequest; ?>
			<?php $temp = new SkinTemplate(); ?>
			<?php if ( !$this->data['isSpecialPage'] ) { ?>
				<?php if ( $wgUser->isLoggedIn() && !$temp->getTitle()->isMainPage() ) { ?>
					<div id="mw-mf-edit-page-link">
						[ <a href="<?php echo $wgRequest->appendQuery( "action=history" ); ?>">history</a> ]
						[ <a href="<?php echo $wgRequest->appendQuery( "action=edit" ); ?>">edit</a> ]
					</div>
				<?php } ?>
				<div id="content">
			<?php } ?>
	...
	public function execute() {
		...
		$this->set( 'bodyClasses', 'mobile beta' );
...
urchinTracker();
	...
	public function navigationStart() {
		?>
		<?php global $wgUser, $wgScriptPath, $wgEmergencyContact; ?>
		<div id="mw-mf-viewport">
		<div id="mw-mf-page-left">
		<div id='mw-mf-content-left'>
		<ul id="mw-mf-menu-main"><?php // GCChange[4] ?>
			<li class='icon14'>
				<a id="mw-mf-display-toggle" href="<?php $this->text( 'viewNormalSiteURL' ) ?>">
					<?php $this->msg( 'mobile-frontend-view-desktop' ) ?>
				</a>
				
			<li class="icon10">
				<a href="<?php echo $wgScriptPath . "/index.php?title=" . Title::newFromText( $this->data['title'] )->getTalkPage(); ?>">
					<?php $this->msg('talk'); ?>
				</a>
			</li>
			...
			<?php if ( $wgUser->isLoggedIn() ) { ?>
				<li class='icon1'>
					<a href="<?php $this->text( 'favoritesUrl' ) ?>">
						<?php $this->msg( 'favoritelist' ) ?>
					</a>
				</li>
			<?php } ?>
			...
			<li class="icon11">
				<a href="<?php echo $wgScriptPath . "/index.php?title=Help:Contents" ?>">
					<?php $this->msg('help'); ?>
				</a>
			</li>
			...
			<?php if ( !$wgUser->isLoggedIn() ) { ?>
			<li class="icon4">
				<a href="mailto:<?php echo $wgEmergencyContact ?>">
					<?php $this->msg('contactus'); ?>
				</a>
			</li>
			<?php } ?>
	...
	public function prepareData() {
		...
		if ( $this->data['isMainPage'] && $this->data['action'] != 'favorite' && $this->data['action'] != 'unfavorite' ) {
			...
			// } elseif ( $this->data['isBetaGroupMember'] && !$this->data['isSpecialPage'] && !$this->data['isMainPage'] && !$editMode ) { // GCChange[1]
			} elseif ( !$this->data['isSpecialPage'] && !$this->data['isMainPage'] && !$editMode ) {
	...
	<div id="mw-mf-header">
		...
		?>
		<?php // GCChange[5] ?>
		<?php global $wgUser, $wgRequest, $wgScriptPath; ?>
		<?php $temp = new SkinTemplate(); ?>
		<form id="mw-mf-searchForm" <?php echo $wgUser->isLoggedIn() ? 'style="padding-right: 40px;"' : ''; ?> action="<?php $this->text( 'scriptUrl' ) ?>" class="search_bar" method="get">
		...
		<?php if ( $wgUser->isLoggedIn() ) { ?>
			<?php $favitem = FavoritedItem::fromUserTitle( $wgUser, $temp->getTitle() ); ?>
			<?php $mode = $favitem->isFavorited() ? 'unfavorite' : 'favorite'; ?>
			<?php $favimg = $mode == 'unfavorite' ? 'unfavorite.png' : 'favorite.png' ?>
			<div id="mw-mf-favdiv">
				<a id="mw-mf-favlink" href="<?php echo $temp->getTitle()->getLocalUrl( 'action=' . $mode ); ?>" style="background-image: url('<?php echo $wgScriptPath . '/extensions/MobileFrontend/stylesheets/modules/images/' . $favimg ?>');"></a>
			</div>
		<?php } ?>
	</div>
	<div id="results"></div>
	...
	private function footer() {
		?>
		<?php global $wgUser, $wgServer, $wgScriptPath; ?>
	<div id="footer">
		<?php
		// @todo: make license icon and text dynamic
		?>
	<h2 class="section_heading openSection" id="section_footer">
		<?php $this->html( 'license' ) ?>
	</h2>
	<div class="content_block openSection" id="content_footer">
		<?php // GCChange[6] ?>
		<ul class="links">
			<li>
				<a id="mw-mf-display-toggle" href="<?php $this->text( 'viewNormalSiteURL' ) ?>">
			...
			<li>
				<?php $this->html( 'privacyLink' ) ?>
			</li>
			<li>
				<?php $this->html( 'aboutLink' ) ?>
			</li>
			<li>
				<?php $this->html( 'disclaimerLink' ) ?>
			</li>
		</ul>
			<!--GCchange Troy T. Lawson Troy.Lawson@tbs-sct.gc.ca add mobile links-->
		</br>
		<ul class="links">
			<li>
				<a id="mw-mf-home-link" href=<?php echo $wgServer.$wgScriptPath ?>>
			...
			<li>
				<a id="mw-mf-logout-link" href="?title=Special:UserLogout">
					<?php $this->msg('mobile-frontend-logout'); ?>
				</a>
			</li>
			<?php } ?>
			<li>
				<a id="mw-mf-contact-link" href="mailto:admin.GCPEDIA@tbs-sct.gc.ca">
					<?php $this->msg('mobile-frontend-leave-feedback-special-title'); ?>
				</a>
			</li>
		</ul>
