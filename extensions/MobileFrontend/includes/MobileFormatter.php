...
class MobileFormatter extends HtmlFormatter {
	...
	/**
	 * GCChange added this private variable to store the titles of pages in which it is intended to have already opened sections on load
	 * @var TitlesToHaveExpandedSections
	 */
	private $titlesToHaveExpandedSections = array(
			...
			case 'HTML':
				if ( $this->expandableSections && !$this->mainPage && strlen( $html ) > 4000 ) {
					...
					if ( $this->removeSections ) {
						...
						// $doc = $temp->getDoc(); // GCChange - commented out this line and the next 5 lines so that the gcchange that makes level 1 headings work as well as level 2 headings works
	...
	private function backToTopLink( $headingNumber ) {
		...
		/**
		 * GCChange - added this so that it is possible to have some pages that have open sections on load
		 */
		$temp = new SkinTemplate();
		$exceptions = $this->titlesToHaveExpandedSections;
		$append = ( ( in_array( $temp->getTitle(), $exceptions ) ) ? ' openSection' : '' );

		return Html::rawElement( 'a',
				array( 'id' => 'anchor_' . $headingNumber,
					'href' => '#section_' . $headingNumber,
					'class' => 'section_anchors back_to_top' . $append ), // GCChange - added $append for open section by default for some pages
	...
	private function headingTransformCallbackHTML( $matches ) {
		...
		$base .= Html::openElement( 'h2',
				array( 'class' => 'section_heading' . $append, 'id' => 'section_' . $this->headings ) // GCChange - added $append for open section by default for some pages
			);
		$base .=
			Html::rawElement( 'span',
				...
				. Html::openElement( 'div',
					array( 'class' => 'content_block' . $append, 'id' => 'content_' . $this->headings ) // GCChange - added $append for open section by default for some pages
				);
	...
	protected function headingTransform( $s ) {
		...
		$s = '<div id="content_0" class="content_block openSection">'
			...
			'%<h[1|2](.*)<span class="mw-headline" [^>]*>(.+)</span>[\s\r\n]*</h[1|2]>%sU', // GCChange - h2 => h[1|2] : makes level 1 headings work as well as level 2 headings
			array( $this, $callback ),
			$s
		);
