/*
 * Section Edit Links for GCVector
 */
( function ( $, mw ) {

var eventBase = 'ext.gcvector.sectionEditLinks-bucket:';
var cookieBase = 'ext.gcvector.sectionEditLinks-';
var bucket = null;

if ( mw.config.get( 'wgGCVectorSectionEditLinksBucketTest', false ) ) {
	// If the version in the client's cookie doesn't match wgGCVectorSectionEditLinksExperiment, then
	...
	var experiment = Number( mw.config.get( 'wgGCVectorSectionEditLinksExperiment', 0 ) );
	...
	if ( bucket === null ) {
		...
		var odds = Math.min( 100, Math.max( 0,
			Number( mw.config.get( 'wgGCVectorSectionEditLinksLotteryOdds', 0 ) )
		) );
...
$(document).ready( function () {
	...
	if ( bucket == 2 ) {
		...
		$( 'span.mw-headline' ).each( function () {
			$(this)
				.after(
					$( '<span class="editsection gcvector-editLink"></span>' )
						.append(
							$(this)
