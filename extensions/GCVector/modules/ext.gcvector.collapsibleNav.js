/**
 * Collapisble navigation for GCVector
 */
( function ( mw, $ ) {
	"use strict";
	var map, version;
	...
	function toggle( $element ) {
		$.cookie(
			'gcvector-nav-' + $element.parent().attr( 'id' ),
	...
	if ( mw.config.get( 'wgCollapsibleNavForceNewVersion' ) ) {
		...
		if ( mw.config.get( 'wgCollapsibleNavBucketTest' ) ) {
			...
			version = $.cookie( 'gcvector-nav-pref-version' );
			...
			if ( version === null ) {
				...
				$.cookie( 'gcvector-nav-pref-version', version, {
					expires: 30,
					path: '/'
				});
			}
	...
	$( function ( $ ) {
		...
		if ( version === 2 ) {
			...
			if ( $secondary.children().length === 0 ) {
				...
				$( '#p-lang-more h5' ).text( mw.msg( 'gcvector-collapsiblenav-more' ) );
		...
		$( '#mw-panel > .portal:not(.persistent)' )
			.each( function ( i ) {
				...
				// Re-save cookie
				if ( state !== null ) {
					$.cookie( 'gcvector-nav-' + $(this).attr( 'id' ), state, { 'expires': 30, 'path': '/' } );
				}
			} );
