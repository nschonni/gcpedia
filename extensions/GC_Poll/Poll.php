...
# with: include("extensions/GC_Poll.php");
$wgExtensionMessagesFiles['GC_Poll'] = dirname( __FILE__ ) . '/Poll.i18n.php';
$wgExtensionFunctions[] = "wfGCPoll";
$wgExtensionCredits['parserhook'][] = array(
	'name'           => 'GCPoll',
	'description'    => 'Anonymous polling',
	'author'         => '[http://www.gcpedia.gc.ca/wiki/User:Matthew.april Matthew April]',
);

function wfGCPoll() {
        global $wgParser; global $wgOut;
        # register the extension with the WikiText parser
        # the first parameter is the name of the new tag.
        # In this case it defines the tag <Poll> ... </Poll>
        # the second parameter is the callback function for
        # processing the text between the tags
        $wgParser->setHook( "GC_poll", "renderGCPoll" );
        #$wgParser->disableCache(); ###############Causing Parser Error###################
}

# The callback function for converting the input text to HTML output
# $argv is an array containing any arguments passed to the extension like <example argument="foo" bar>..

function renderGCPoll( $input, $argv=array() ) {
        global $wgParser,$wgUser,$wgLang,$wgScriptPath,$wgTitle,$wgRequest,$wgArticlePath,$wgOut,$wgPollCount;
        $wgImagePath=$wgScriptPath . "/extensions/GC_Poll/images";
		
		//echo "<pre>";
		//print_r( $wgUser );
		//echo "</pre>";
		
		$wgPollCount++;//Count number of polls used per page (so we can limit to one)
		$maxPolls = 1;
		...
		$pageTitle = $wgTitle;// GCEDIT
        $ID = strtoupper(md5($pageTitle)); 	// ID of the poll based on page title
        $err = "";
		...
		if($wgPollCount == $maxPolls)// limit 1 poll per page, dispaly limit message otherwise
		{
			...
			if (!empty($_POST['poll_ID']))  // POST Variables treatments
					{
					  ...
					  if ($isLoggedIn) // PROCESS VOTE ONLY FOR LOGGED USERS
			...
			// has the user already voted ? ($deja_vote)
	...
	and TIME_TO_SEC(title.poll_datemaxi)-TIME_TO_SEC(poll_info.poll_date)>2';
			...
			for ($i=2;$i<$nbansw;$i++)
				...
				{
				$str .=($locked!=''?'':
				'<input type="image" align=texttop src="'.$wgImagePath.'/box-check.png" name="Vote" alt="Vote" name="Vote" '.
						($small==''?'width=18 height=15':'width=18 height=15').'> ').
						ucfirst($lines[$i]).'  '.$smalltxt2.'</form></td>';
				}else{
				$str .= ($locked!=''?'':
				 '<input type="image" align=texttop src="'.$wgImagePath.'/box.png" name="Vote" alt="Vote" name="Vote" '.
						($small==''?'width=18 height=15':'width=18 height=15').'> ').
						ucfirst($lines[$i]).'  '.$smalltxt2.'</form></td>';
				}
			...
			if($deja_vote > 0)
			{
					$str .=	"<div style='background-color:#ddd; color:#0A0; font-size:12px;'>" .
							wfMsg('votesubmitted') .
							"</div>";
			}
			...
			if(!$isLoggedIn)
			{
				$str .=	"<div style='background-color:#ddd; color:#F00;font-size:12px;'>" .
						wfMsg('pleaselogin') .
						"</div>";
			}
			...
			$str = "Limit of <strong>{$maxPolls}</strong> private poll per page";//Needs translation
		}
