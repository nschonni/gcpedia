...
global $wgUser;
//die('id='. $id. "  units=" . $units . "  static=" . $static);
...
if ($wgUser->mOptions['language'] == 'fr') {
	$tense=($count==1) ? "voix" : "voix"; //plural form votes/vote	
}
else {
...
if ($static == 'static') {
      ...
      for ($ncount = 1; $ncount <= $units; $ncount++) { // loop from 1 to the number of units
           if(!$voted) { // if the user hasn't yet voted, draw the voting stars
              $rater.='<li><a href="/extensions/Ratings/db.php?j='.$ncount.'&amp;q='.$id.'&amp;t='.$ip.'&amp;c='.$units.'" title="'.$ncount.' out of '.$units.'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>';
           }
      }
	  ...
	  if ($wgUser->mOptions['language'] == 'fr') {
		 $rater.='>'.$id.' &eacute;valuant: <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' &eacute;mises)';
	  }
	  else {
