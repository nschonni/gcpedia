<?php
	...
	require_once(dirname(__FILE__)."/accesscontrolSettings.php");
	...
	function doRedirect()
	{
		...
		// some first initialisations
		if (trim($wgAccessControlNoAccessPage)=="") $wgAccessControlNoAccessPage=GCPath::pageURL('No_Access');
	...
	function controlUserAccess( $input, $showGroupText, $useReadOnlyFlag = false )
	{
		...
		// the user type that can edit the page
		global $wgAccessControlEditGroupPrefix;
		...
		if (!in_array(strtolower(trim($wgUser->getName())), $allowedAccess))
		{
			...
			if (((in_array("sysop", $wgUser->mGroups)) || (in_array($wgAccessControlEditGroupPrefix, $wgUser->mGroups))) && ($wgAdminCanReadAll == true))
			{
	...
	function controlMediaWikiUserAccess( $input, $showGroupText, $useReadOnlyFlag = false )
	{
		...
		if((in_array("sysop", $wgUsermGroups) || in_array($wgAccessControlEditGroupPrefix, $wgUser->mGroups)) && $wgAdminCanReadAll)
		{
        ...
        function parseLink($link)
	{
		...
		// return the HTML link
                //return "<a href=\"/.php/$linkTitle\" title=\"$linkTitle\">$linkName</a>";
                return "<a href='".GCPath::pageURL($linkTitle)."' title='$linkTitle'>$linkName</a>"; 
        }
	...
	function controlUserGroupPageAccess( $out )
	{
		...
		{
			if ((!in_array("sysop", $wgUser->mGroups)) && (!in_array($wgAccessControlEditGroupPrefix, $wgUser->mGroups)))
			{
