...
define( 'CATEGORYWATCH_VERSION', 'GC 1.2.2, 2011-06-01' );
...
$wgHooks['GetPreferences'][] = "CategoryWatch::wfCategoryWatchPrefs";
...
class CategoryWatch {
	...
	public static function wfCategoryWatchPrefs( $user, &$preferences ) {
		...
		$preferences['CategoryWatchEmailPref'] = array(
			'type' => 'toggle',
			'label-message' => 'categorywatch-emailprefs',
	...
	function notifyWatchers( &$title, &$editor, &$message, &$summary, &$medit ) {
		...
		#GCEDIT - $watchingUser->getOption( 'enotifwatchlistpages' )
			$userLanguage = $watchingUser->getOption( 'language' );        // in user's language

          		  if ( $watchingUser->getOption( 'enotifwatchlistpages' ) && !$watchingUser->getOption( 'CategoryWatchEmailPref' ) && $watchingUser->isEmailConfirmed() ) {
		                $to      = new MailAddress( $watchingUser );
              		  $subject = wfMsg( 'categorywatch-emailsubject', $page );
		                $body    = wfMessage( 'enotif_body' )->inLanguage( $userLanguage )->text();

				# Reveal the page editor's address as REPLY-TO address only if
				# the user has not opted-out and the option is enabled at the
				# global configuration level.
				$name = $wgEnotifUseRealName ? $watchingUser->getRealName() : $watchingUser->getName();
				$editorName = $wgEnotifUseRealName ? $editor->getRealName() : $editor->getName();
				...
				$keys = array(
					...
					'$UNWATCHURL'		=> $title->getFullUrl() . '?action=unwatch',
					'$PAGEEDITOR_WIKI'  => $userPage->getFullUrl(),
					'$PAGESUMMARY'      => $summary,
					'$PAGEMINOREDIT'    => $medit,
					'$OLDID'            => ''
				);
				if ( $editor->isIP( $editorName ) ) {
					$utext = wfMsgForContent( 'enotif_anon_editor', $editorName );
					$subject = str_replace( '$PAGEEDITOR', $utext, $subject );
					$keys['$PAGEEDITOR'] = $utext;
					$keys['$PAGEEDITOR_EMAIL'] = wfMsgForContent( 'noemailtitle' );
				} else {
					$subject = str_replace( '$PAGEEDITOR', $editorName, $subject );
					$keys['$PAGEEDITOR'] = $editorName;
					$emailPage = SpecialPage::getSafeTitleFor( 'Emailuser', $editorName );
