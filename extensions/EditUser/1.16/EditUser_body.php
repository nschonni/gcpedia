...
class EditUser extends SpecialPage {
	...
	function mainPrefsForm( $status , $message = '' ) {
		...
		$wgOut->addHTML(
			Xml::fieldset( wfMsg( 'prefs-files' ) ) . "\n" .
			Xml::openElement( 'table' ) .
				'<tr>
					<td class="mw-label">' .
						Xml::label( wfMsg( 'prefs-imagemaxsize' ), $imageSizeId ) .
					'</td>
					<td class="mw-input">' .
		...
		$wgOut->addHTML(
			...
			Xml::element( 'legend', null, wfMsg( 'prefs-datetime' ) ) . "\n"
		);

		if ($dateopts) {
			$wgOut->addHTML(
				Xml::openElement( 'fieldset' ) .
				Xml::element( 'legend', null, wfMsg( 'prefs-dateformat' ) ) . "\n"
			);
			$idCnt = 0;
		...
		$wgOut->addHTML(
			$this->addRow(
				Xml::label( wfMsg( 'timezonelegend' ), 'wpTimeZone' ),
				$opt )
		);
		$wgOut->addHTML(
			$this->addRow(
		...
		$wgOut->addHTML(
			Xml::fieldset( wfMsg( 'prefs-textboxsize' ) ) .
