...
class EditUser extends SpecialPage {
	...
	function execute( $par ) {
		...
		// GCChange - use $this->getContext() to fix the error caused by 'EditUserPreferencesForm'
		$htmlForm = Preferences::getFormObject( $targetuser, $this->getContext() /*'EditUserPreferencesForm'*/ );
	...
	function tryUISubmit( $formData ) {
		...
		// GCChange - copied $htmlForm into this function in order to fix the error caused by 'uiEditUser'
		$htmlForm = Preferences::getFormObject( $targetuser, $this->getContext() /*'EditUserPreferencesForm'*/ );
		$htmlForm->setSubmitCallback( array( $this, 'tryUISubmit' ) );
		$htmlForm->setTitle( $this->getTitle() );
		$htmlForm->addHiddenField( 'username', $this->target );
		...
		$realUser = $wgUser;
		$wgUser = $targetuser;
		$res = Preferences::tryFormSubmit( $formData, $htmlForm /*'uiEditUser'*/ );
		$wgUser = $realUser;
