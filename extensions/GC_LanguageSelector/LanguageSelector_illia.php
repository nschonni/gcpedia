...
function wfLanguageSelectorHTML( $style = NULL, $class = NULL, $selectorstyle = NULL, $buttonstyle = NULL) {
	...
	foreach ($wgLanguageSelectorLanguages as $ln) 
	{
		$url = "";
		if ($url == "")
			$url = GCPath::pageURL($wgTitle,"setlang=$ln");
		
		if ($ln == $code) 	$output .= "<div class='langLink' style='color: #fff;font-weight: bold;' lang='fr' xml:lang='$ln'>".$wgContLang->getLanguageName($ln)."</div>";
		else 				$output .= "<a class='langLink' style='color: #fff;' lang='en' xml:lang='$ln' href=\"".$url."\">".$wgContLang->getLanguageName($ln)."</a>";
...
/*
	...
	-----------------------------------------------------------------------------------------------------------------------
*/
function onUserCreation($user, $byEmail)
{
