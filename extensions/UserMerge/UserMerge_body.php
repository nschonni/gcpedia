...
class UserMerge extends SpecialPage {
	 ...
	 * @author Matthew April <Matthew.April@tbs-sct.gc.ca>
	 *
	 * EDIT: this function has an error, was rewritten by Daniel Prodan to work properly
	 */
	private function mergeEditcount( $newuserID, $olduserID ) {
		...
		//get edit num from new user
		$queryString = "SELECT * FROM `user` WHERE `user_id` = '".$newuserID."'";
		$result = $dbw->fetchRow($dbw->query($queryString));
		
		$newEdits = $result[14];
		
		//get edit num from old user
		$queryString = "SELECT * FROM `user` WHERE `user_id` = '".$olduserID."'";
		$result = $dbw->fetchRow($dbw->query($queryString));
		...
		$queryString = "UPDATE IGNORE `user` SET `user_editcount` = ".$totalEdits." WHERE `user_id` = '".$newuserID."'";
		$dbw->query($queryString);
		
		//sets old user edits to 0, incase old user is not deleted
		$queryString = "UPDATE IGNORE `user` SET `user_editcount` = 0 WHERE `user_id` = '".$olduserID."'";
		$dbw->query($queryString);
